...
const MeusEventos = () => {
  ...
  return (
    <section className="painel-eventos">
      ...
      <p>Olá, Maria! Aqui estão os ingressos dos eventos que você comprou.</p>

      <div className="grade-eventos">
        {eventos.map((evento) => (
          <div key={evento.id} className={`cartao-evento ${evento.id}`}>
            <h3>{evento.nome}</h3>
            <p><strong>Data:</strong> {evento.data}</p>
            <p><strong>Local:</strong> {evento.local}</p>
            <p className="tempo-restante">{calcularTempoRestante(evento.data)}</p>

            <div className="bloco-ingressos">
              <h4>🎫 Tipos de ingresso:</h4>
              <ul>
                {evento.ingressos.map((ingresso, index) => (
                  <li key={index}>
                    <strong>{ingresso.tipo}</strong> – {ingresso.valor}
                    <span className="etiqueta">{ingresso.nivel}</span>
                  </li>
                ))}
              </ul>
            </div>
            ...
            <div className="acoes-evento">
              <button className="botao-cancelar" onClick={() => cancelarIngresso(evento)}>
                Cancelar ingresso
              </button>
            </div>
          </div>
