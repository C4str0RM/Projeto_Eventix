...
import { todosEventos } from "../../types/index2";
import { novosBanners } from "../../types/index3";
import { destaqueBanners } from "../../types/index4";
...
const Home = ({ usuario }) => {
  ...
  const reiniciarScroll = (ref, direction, passo) => {
    ...
    if (scrollAtual >= scrollMax - passo) {
      ...
      el.scrollBy({
        [direction === "horizontal" ? "left" : "top"]: passo,
        behavior: "smooth",
      });
    }
  };
  ...
  useEffect(() => {
    const intervalH = setInterval(
      () => reiniciarScroll(horizontalRef, "horizontal", 735),
      4000
    );
    const intervalNovos = setInterval(
      () => reiniciarScroll(novosRef, "horizontal", 400),
      4000
    );
    return () => {
  ...
  return (
    <main className="home-container">
      {usuario?.logado && <div className="saudacao-usuario"></div>}

      {/* Destaques */}
      <section className="destaques">
        <h2 className="faixa-titulo">Destaques</h2>
        <div className="carrossel-wrapper">
          <button onClick={() => scroll(horizontalRef, "horizontal", -735)}>
            ←
          </button>
      ...
      <section className="novos-eventos">
        <h2 className="faixa-titulo">Eventos Novos</h2>
        <div className="carrossel-wrapper">
          <button onClick={() => scroll(novosRef, "horizontal", -560)}>
            ←
          </button>
